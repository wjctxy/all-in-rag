...
-   `percentile` (百分位法 - **默认方法**):
    ...
    -   **形象例子**: 把所有句间差异值想象成一次“考试排名”。如果一篇文档里有100个相邻句子差异值，设 `breakpoint_threshold_amount=95` 就等于“只看前5名最异常的跳跃”。也就是：先把100个差异值从小到大排序，取第95百分位对应的阈值 `T`，再把大于 `T` 的位置当作断点。这样不会因为普通波动频繁切分，而是只在最明显的语义突变处切开。
