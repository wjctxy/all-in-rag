...
-   `percentile` (百分位法 - **默认方法**):
    ...
    -   **形象例子**: 假设一段文本有10个相邻句子间的差异值：`[0.05, 0.08, 0.10, 0.12, 0.15, 0.16, 0.18, 0.20, 0.55, 0.90]`。当阈值设为95分位时，系统会先算出一个较高阈值 `T`，然后只保留 `> T` 的点作为断点。直观上，它等价于“只留最靠后的少数极端值”，因此这个例子里最明显会被选中的就是 `0.90`；如果把阈值降到90分位，被选中的断点会增多。可以把它理解成“只挑全班最异常的几次跳跃来切分”，而不是见到一点变化就切。
