...
-   `percentile` (百分位法 - **默认方法**):
    ...
    -   **形象例子**: 假设一段文本有10个相邻句子间的差异值：`[0.05, 0.08, 0.10, 0.12, 0.15, 0.16, 0.18, 0.20, 0.55, 0.90]`。当阈值设为95分位时，阈值会落在“最靠后的少数大值”区间（接近 `0.90`，且高于大多数值）。这时会优先命中 `0.90` 这类极端跳跃值；是否同时命中 `0.55`，取决于具体的分位数计算与比较实现（如果阈值是90分位，`0.55` 和 `0.90` 更容易同时被选中）。可以把它理解成“只挑全班最异常的几次跳跃来切分”，而不是见到一点变化就切。
